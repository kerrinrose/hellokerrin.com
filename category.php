<?php get_header(); ?>
<h1 class="fade-1 w-full  pt-6 sm:text-4xl md:text-4xl tracking-wide font-munro load-hidden">all
...
<ul class=" fade-1 list-reset">
  ...
  ?>
    ...
    <li class=" py-4">
        <?php if($thumbnail) { ?><img class="h-48 xl:h-64 w-full <?php echo $object_pos ?>"
            src="<?php echo $thumbnail ?>"><?php } ?><br>
        ...
        <h3 class="font-normal leading-normal pb-3"><?php echo get_post_meta($post->ID, "skills", true); ?> for <a
                href="<?php if($external) { echo $external;} else { the_permalink();} ?>"><?php echo $title ?></a></h3>
